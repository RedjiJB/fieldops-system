...
**With a domain — what this repo actually runs now** (a stable, permanent URL, switched over 2026-08-16 once `sodboysltd.org` was registered):
1. Add the domain to your Cloudflare account (any registrar is fine, as long as DNS is delegated to Cloudflare)
2. Create a tunnel: Cloudflare dashboard → Zero Trust → Networks → Tunnels → Create a tunnel → Cloudflared (this repo's is named `fieldops-dashboard`)
...
5. In `docker-compose.yml`'s `cloudflared` service: `command: tunnel --no-autoupdate --protocol http2 run`, `environment: [TUNNEL_TOKEN=${CLOUDFLARE_TUNNEL_TOKEN}]` (the `--protocol http2` carries over from the Quick Tunnel era below — same NAT-timeout reasoning, applies either way)
6. `docker compose up -d cloudflared`

**Without a domain** (Quick Tunnel — what this repo ran before 2026-08-16, and the fallback if you're setting this up fresh without a domain yet): `docker-compose.yml`'s `cloudflared` service runs `tunnel --no-autoupdate --protocol http2 --url http://frontend:80` instead, no token needed. Cloudflare generates a random `https://<random-words>.trycloudflare.com` URL, printed to `docker compose logs cloudflared` on every start. Real tradeoffs, not just a formality: the URL **changes on every container restart** (Pi reboot, `docker compose restart`, anything), and there's no login/access control on the tunnel itself beyond the dashboard's own auth — anyone with the URL who guesses/finds it reaches the login page. Fine as a stopgap for one person checking a map, not something to hand out to a crew. Switch to the domain-based setup above once a domain exists — the swap is just the `command`/`environment` block in `docker-compose.yml`.

**Confirmed the hard way**: Quick Tunnel's "no uptime guarantee" isn't just disclaimer text — it silently died for ~30 hours (75 failed reconnection attempts, container never crashed so `docker compose ps` kept showing it as running) with nothing to catch it, and separately its QUIC/UDP transport was found to drop every ~8-13 minutes on this host's network (a home router's UDP NAT table timing out an idle connection) — `--protocol http2` fixes that specific failure mode, but not the URL-rotation tradeoff, which only the named-tunnel setup above fixes. `openclaw/notifier/sync-dashboard-url.mjs` (a cron job, every 5 min) polls reachability, keeps a live URL cached for the agent's `get_dashboard_url` tool, and pages IT (`dashboard_unreachable` alert) if it's down — see `docs/ARCHITECTURE.md`'s Hosting section and `docs/EXCEPTION_HANDLING.md`. Still useful with a named tunnel (the URL stays fixed, but the tunnel can still drop), and `openclaw/notifier/heartbeat.mjs` (added the same day) covers the broader case of the backend/Postgres/connectivity itself going down, which the dashboard-url check alone can't catch. The `cloudflared` service also has an explicit `logging:` block (`max-size: 10m`, `max-file: 3`) — `docker compose logs` output on a `restart: unless-stopped` service polled every few minutes would otherwise grow unbounded.
