...
**Confirmed the hard way**: Quick Tunnel's "no uptime guarantee" isn't just disclaimer text — it silently died for ~30 hours (75 failed reconnection attempts, container never crashed so `docker compose ps` kept showing it as running) with nothing to catch it. `openclaw/notifier/sync-dashboard-url.mjs` (a cron job, every 5 min) now polls it, keeps a live URL cached for the agent's `get_dashboard_url` tool, and pages management (`dashboard_unreachable` alert) if it's down — see `docs/ARCHITECTURE.md`'s Hosting section and `docs/EXCEPTION_HANDLING.md`. The `cloudflared` service also now has an explicit `logging:` block (`max-size: 10m`, `max-file: 3`) — it had none before, and `docker compose logs` output on a `restart: unless-stopped` service now being polled every 5 minutes would otherwise grow unbounded.
