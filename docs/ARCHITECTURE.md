...
Currently running Cloudflare's **Quick Tunnel** mode (no domain registered), which mints a new random URL on every restart and has no uptime guarantee — confirmed the hard way when it silently died for ~30 hours (75 failed reconnection attempts, no monitoring on the tunnel's own health) before anyone noticed. `openclaw/notifier/sync-dashboard-url.mjs` now polls it every 5 minutes, keeps a live URL cached in Postgres (`dashboard_url` table) for the agent's `get_dashboard_url` tool to read, and raises a `dashboard_unreachable` alert (see [EXCEPTION_HANDLING.md](EXCEPTION_HANDLING.md)) when it isn't reachable. The agent can also restart it directly on request (`restart_dashboard_tunnel`) — the one tool in `fieldops-tools` that shells out to `docker` on the host instead of calling the backend, since the OpenClaw gateway runs as a native systemd service on the Pi, not a container, and already has that access. Switching to the stable named-domain tunnel `DEPLOYMENT.md` documents remains the actual fix for this class of problem; the polling/restart machinery is a mitigation on top of Quick Tunnel, not a replacement for it.
