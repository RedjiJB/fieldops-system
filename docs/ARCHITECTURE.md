...
**Crew dashboard access (added, v1 scoped narrow).** Every crew member now has *some* dashboard view, not just management/admin — but true to principle 1, getting there is still "almost no typing": a crew member asks the agent for the link over WhatsApp, gets a one-time login link back (`send_dashboard_login_link`, 15-minute expiry, single use), taps it, and lands in a small scoped view of their own pay, jobs/shifts, checkouts, and spend claims (`/me/*` routes, `backend/src/routes/me.ts` — see [API.md](API.md#my-stuff-crew-self-service)). This is a genuinely separate auth path from the dashboard's email+password login (`sessions` is dual-path as of `0051`/`0052` — see [DATABASE_SCHEMA.md](DATABASE_SCHEMA.md#dashboard-auth)), and a genuinely separate frontend surface (`CrewPortalPage`, not a cut-down version of the admin `Dashboard`). What's **not** built yet: foreman's "a little more" (site-level crew/inventory visibility) and management's "even more" (broader operational access) — only the crew tier and the reusable auth foundation exist so far. This is also the first place in the API that does row-level scoping server-side (deriving the caller's identity from their session, never a client-supplied id) rather than trusting a client-supplied `crew_member_id` filter like every other dashboard route does.
