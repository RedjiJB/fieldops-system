...
This is the original pre-build plan — phases 0–7 below are what was scoped before any code existed. The system has since grown past this list (geofence verification, shift reminders, crew-level live location, shift extension, carpool, and the message-draft review queue all landed after phase 7 and aren't reflected in the table below). See [CHANGELOG.md](../CHANGELOG.md) for what was actually built, in order, [SECURITY.md](SECURITY.md) for the "what's fixed vs. still open" status, and [ROADMAP_NEXT.md](ROADMAP_NEXT.md) for post-v1 candidates (v1.1–v1.9). This file is kept as-is for the historical planning record, not updated to track current scope.
