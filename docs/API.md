...
| `GET` | `/pending-confirmations?status=&whatsapp_message_id=` | `admin`-only for a dashboard session; the service token passes through ungated (same trust boundary as `GET /notifications/pending`) so the agent can look up what's open. `whatsapp_message_id` resolves "the thing management just quote-replied to," joined through the linked notification. |
| `PATCH` | `/pending-confirmations/:id/approve` | Dashboard: `admin`/`owner` only, `reviewed_by_user_id` set. Service token: `{reviewed_by}` (a crew member id) required — 404 if it doesn't exist, 403 if `role` isn't `management` or `owner`, else `reviewed_by` set. Either path also takes `{rate_per_km?}`, required only for `action_type: 'mileage_claim'`. Re-validates against current state (e.g. re-runs the timeclock legal-transition check) before dispatching to the real mutation; 400 if not `awaiting_management`. Also acknowledges the linked notification (`acknowledged_by`/`acknowledged_by_user_id`, same dual path). |
