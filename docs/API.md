...
| `PATCH` | `/system/dashboard-url` | Service-token only. `{url}` — called by `openclaw/notifier/sync-dashboard-url.mjs` when it captures a new tunnel URL from `cloudflared`'s logs (still relevant on the current named tunnel — the connector can register a fresh session URL segment on reconnect even though the hostname itself is now stable). |
