...
- `plugins/fieldops-tools/` — the agent tool plugin mapping the backend API ([../docs/API.md](../docs/API.md)) to callable tools, built with OpenClaw's `defineToolPlugin` SDK (`openclaw plugins init --type tool`). **Assets & Inventory group done** (8 tools: list/get/register/verify/update-status assets, list/adjust consumables, site inventory) — built, `npm run plugin:build && npm run plugin:validate` passing, installed with `openclaw plugins install ./plugins/fieldops-tools --link`, and smoke-tested end to end against the real backend (register → verify → correctly filtered by status). Remaining groups (Loadouts & Checkout, Orders, Vendors/PO, Scheduling, Alerts, Vehicles/Location, Documents) not yet built.
