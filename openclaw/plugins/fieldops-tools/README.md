...
All 61 tools from every docs/API.md group are wired up and smoke-tested against the live backend:
...
- **System** — `get_dashboard_url` (current Cloudflare Quick Tunnel URL + last health check, never a static string — see `openclaw/notifier/sync-dashboard-url.mjs`), `restart_dashboard_tunnel` (the one tool here that shells out to `docker` on the host directly instead of calling the backend — see its code comment)
